...
function handleDeletFile(deletFile){
 ...
 async function handlePostListing(){
     ...
     if(listingData.description.length < 10 ){
          setListingError("description must be atleast 20 character for a document");
          return;
     }
     try {
          ...
          <div className="container listing-container rounded-2 w-75 mt-4 bg-white p-3 py-6 d-flex gap-4 justify-content-center align-items-center flex-column ">
               ...
               <div className="row  gutter gy-4 w-100">
                         <div className="col-12 col-lg-7 row-cols-7 w-full  d-flex gap-2 justify-content-center align-items-center flex-column ">
                              <input type="text" id='name' onChange={(e)=>setlistingData({...listingData,name:e.target.value})} value={listingData.name} className='w-100 p-2  rounded-1  border-1 form-control' placeholder='name' />
                              <input type="text" id='address' onChange={(e)=>setlistingData({...listingData,address:e.target.value})} value={listingData.address} className='w-100 p-2 rounded-1  border-1 form-control' placeholder='address' />
                              <textarea onChange={(e)=>setlistingData({...listingData,description:e.target.value})} value={listingData.description} className='w-100 description-box p-2 rounded-1  border-1 form-control resize-none'  id="description" placeholder='description'></textarea>
                              <div className='d-flex gap-3 justify-content-start flex-wrap align-items-center '>
                                    <div className='d-flex justify-content-center gap-1 align-items-center '>
                                        <input type="checkbox" onChange={(e)=>setlistingData({...listingData,type:"sale"})} checked={listingData.type === "sale" ? true:false} id='sale' />
                                        <label htmlFor="sale"  className='text-black'>sale</label>
                                   </div>
                                    <div className='d-flex justify-content-center gap-1 align-items-center '>
                                        <input type="checkbox" onChange={(e)=>setlistingData({...listingData,type:"rent"})} checked={listingData.type==='rent' ? true : false} id='rent' />
                                        <label htmlFor="rent" className='text-black'>rent</label>
                                   </div>
                                    <div className='d-flex justify-content-center gap-1 align-items-center '>
                                        <input type="checkbox" onChange={(e)=>setlistingData({...listingData,parking:e.target.checked ? true : false})} checked={listingData.parking} id='parking' />
                                        <label htmlFor="parking" className='text-black'>parking</label>
                                   </div>
                                    <div className='d-flex justify-content-center gap-1 align-items-center '>
                                        <input type="checkbox" onChange={(e)=>setlistingData({...listingData,furnished:e.target.checked ? true : false})} checked={listingData.furnished} id='furnished' />
                                        <label htmlFor="furnished"  className='text-black'>furnished</label>
                                   </div>
                                    <div className='d-flex justify-content-center gap-1 align-items-center '>
                                        <input type="checkbox" onChange={(e)=>setlistingData({...listingData,offer:e.target.checked ? true : false})} checked={listingData.offer} id='offer' />
                              ...
                              <div className='d-flex w-100 justify-start mt-2 align-items-center gap-3   '>
                                        <div className='d-flex justify-content-start align-items-center gap-2 '>
                                                  <input type="number" onChange={(e)=>setlistingData({...listingData,bed:Number(e.target.value)})} value={listingData.bed} className='form-control w-50  ' id='bed' min={1}/>
                                                  <label htmlFor="bed" className='text-black'>bed</label>
                                        </div>
                                        <div className='d-flex justify-content-start align-items-center gap-2 '>
                                                  <input type="number" onChange={(e)=>setlistingData({...listingData,bath:Number(e.target.value)})} value={listingData.bath} className='form-control w-50 ' id='bath' min={1}/>
                                                  <label htmlFor="bath" className='text-black'>bath</label>
                                        </div>
                              </div>
                              <div className='w-100 mt-4   '>
                                        <div className=' w-75  gap-3 d-flex  align-items-center '>
                                               <input type="text"  onChange={(e)=>{setlistingData({...listingData,regularPrice:e.target.value})}} value={listingData.regularPrice}  id='regularPrice' className=' form-control m-0' placeholder='regularPrice' />   
                                       ...
                                       {
                                             listingData.offer &&  <div className=' w-75 mt-3  gap-3 d-flex  align-items-center '>
                                               <input type="text"  onChange={(e)=>{setlistingData({...listingData,discountPrice:e.target.value})}} value={listingData.discountPrice}  id='disscountPrice' className=' form-control m-0' placeholder='descountPrice' />   
                                               <span className='text-black'>
                                                  {
                                                       listingData.type==='rent' ? "($/month)":"($/sale)"
                                                  }
               ...
               <div className='w-75'>
                    {
                         ListingError && 
                         <div className='alert alert-danger w-100 d-flex justify-content-between  align-items-center ' role='alert'>
                ...
                <div className='w-100 text-center mt-3'>
                    <button onClick={handlePostListing} className='w-75 btn p-2 rounded-2 text-white ' style={{backgroundColor:"var(--slate-color)"}}> 
                         {
                          ListingLoading ?  <div className='spinner-border '>
